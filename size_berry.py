...
# ---- config ----
CAM_PREFIX = "ZEDXMini_SN50918724"
ROOT = Path("./berry_dataset"); RAW = ROOT/"raw"; LBL = ROOT/"labels"; CSV = ROOT/"metadata.csv"
...
def pick_serial(prefix: str) -> int:
    for d in sl.Camera.get_device_list():
        if d.device_name.startswith(prefix): return d.serial_number
    print(f"No camera with prefix '{prefix}'"); sys.exit(1)
